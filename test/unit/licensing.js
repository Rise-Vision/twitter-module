...
describe("Licensing - Unit", ()=> {
  ...
  beforeEach(() => {
    ...
    mock(messaging, "retryAfterStartup").returnWith();
  ...
  it("should call retryAfterStartup after calling for licensing and display info data", done => {
    ...
    .then(() => {
      ...
      assert.equal(2, messaging.retryAfterStartup.callCount);
      assert.equal(tenMinutesInMilliseconds, messaging.retryAfterStartup.calls[0].args[1]);
      assert.equal(tenMinutesInMilliseconds, messaging.retryAfterStartup.calls[1].args[1]);
      done();
    })
    .catch(error => {
      assert.fail(error);
